...
function animate() {
  // cycleOne.tails.slice().forEach(tail => { 
  //   if (cycleOne.isCollidingWith(tail)) {
  //     isGameOver = true;
  //   }});
  cycleOne.draw(ctx);
  cycleOne.move();
...
function keyDownHandler(e) {
  switch (e.keyCode) {
    case 38:
      // UP
      if (cycleOne.direction != 'Down'){
        cycleOne.direction = 'Up';
      break;
      }
    case 40:
      // DOWN
      if (cycleOne.direction != 'Up'){
      cycleOne.direction = 'Down';
      break;
      }
    case 37:
      // LEFT
      if (cycleOne.direction != 'Right'){
        cycleOne.direction = 'Left';
        break;
        }
    case 39:
      // RIGHT
      if (cycleOne.direction != 'Left'){
