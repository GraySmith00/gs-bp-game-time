...
class Game {
  ...
  animate(ctx) {
    ...
    this.selfCrash(playerOne);
    this.selfCrash(playerTwo);
  ...
  selfCrash(player) {
    player.lightCycle.tails.slice(1).forEach(tail => {
      if (player.lightCycle.tails[0].isCollidingWith(tail)) {
        console.log(`${player} one crashed into himself!`);
        this.gameOver = true;
      }
    });
  ...
  crashIntoOtherPlayer(playersArray) {
    player.lightCycle.tails.slice(1).forEach(tail => {
      if (player.lightCycle.tails[0].isCollidingWith(tail)) {
        console.log(`${player} one crashed into himself!`);
        this.gameOver = true;
